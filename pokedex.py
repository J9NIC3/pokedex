...
def option_5():
  ...
  type1_index = [i+1 for i, e in enumerate(type1[1:]) if e == input_type1]
  type2_index = [i+1 for i, e in enumerate(type2[1:]) if e == input_type2]

  type1_legendary=[]
  type2_legendary=[]
  for value in type1_index:
    if Legendary[int(value)]=="TRUE":
      type1_legendary.append(value)

  for value in type2_index:
    if Legendary[int(value)]=="TRUE":
      type2_legendary.append(value)

  for value in type1_legendary:
    value = int(value)
    if value in type2_legendary:  #if the pokemon is found in both type1 and type2
      type.append(value)
